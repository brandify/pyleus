...
public class PyleusTopologyBuilder {
    ...
    public static final GeneralComponentsFactory gFactory = new GeneralComponentsFactory();

    public static void handleBolt(final TopologyBuilder builder, final BoltSpec spec,
        final TopologySpec topologySpec) {

        IRichBolt stormBolt;
        if (!spec.type.isEmpty() && !spec.type.equals("python")) {
            //build general bolt
            stormBolt = handleGeneralBolt(builder, spec, topologySpec);
        } else {
            //build python bolt
            stormBolt = handlePythonBolt(builder, spec, topologySpec);
        }
    ...
    public static IRichBolt handlePythonBolt(
            @SuppressWarnings("unused") final TopologyBuilder builder, 
            final BoltSpec spec, final TopologySpec topologySpec) {
    ...
    public static IRichBolt handleGeneralBolt(
            @SuppressWarnings("unused") final TopologyBuilder builder, 
            final BoltSpec spec, final TopologySpec topologySpec) {
        
        GeneralBolt bolt = gFactory.createGeneralBolt(spec.type, spec.module, spec.options, 
    ...
    public static void handleSpout(final TopologyBuilder builder, final SpoutSpec spec,
        ...
        } else if (spec.type.isEmpty() || spec.type.equals("python")) {
            spout = handlePythonSpout(builder, spec, topologySpec);
        } else {
            spout = handleGeneralSpout(builder, spec, topologySpec);
        }
    ...
    public static IRichSpout handleGeneralSpout(
            @SuppressWarnings("unused") final TopologyBuilder builder, 
            final SpoutSpec spec, final TopologySpec topologySpec) {
        
        GeneralSpout spout = gFactory.createGeneralSpout(spec.type, spec.module, spec.options, 
                topologySpec.logging_config, topologySpec.serializer);
    ...
    public static IRichSpout handleKafkaSpout(
        ...
        if (forceFromStart != null) {
            config.ignoreZkOffsets = forceFromStart; // This probably isn't exactly right
        }
